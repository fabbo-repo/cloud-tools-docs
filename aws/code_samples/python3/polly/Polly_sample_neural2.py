...
spoken_text = polly.synthesize_speech(
    ...
    OutputFormat='mp3',
    Text='<speak> Hola! soy Lupe y voy a probar ssml. Hoy es, <say-as interpret-as="date" format="ddmmyyyy" detail="1">'+str(fecha)+'</say-as> y son las <say-as interpret-as="time" format="hms12">'+str(hora)+'</say-as></speak>',
...
spoken_text = polly.synthesize_speech(
    Engine='neural',  # not rquired
    OutputFormat='mp3',
    Text='<speak> Ahora dire 58 como <say-as interpret-as="ordinal">58</say-as></speak>',
    TextType='ssml',    # not rquired
    VoiceId='Lupe')
sp.playMP3(spoken_text['AudioStream'].read())