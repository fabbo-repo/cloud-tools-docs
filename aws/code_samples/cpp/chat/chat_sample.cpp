...
int main(int argc, char* argv[])
{
    ...
    // Loop for chat
    std::string input = "";  std::cout << "Starting chat. \"exit()\" to quit" << std::endl << std::endl;
    while(input != "exit()") {
        std::cout << "Request message: ";
        std::cin >> input;
        if(input == "exit()") break;

        Aws::Vector<Aws::LexRuntimeV2::Model::Message> messages = recognizeText(input);
        std::cout << "Number of responses: " << messages.size() << std::endl;
        for (int i = 0; i < messages.size(); i++ )
            std::cout << "Response message N" << i << ": " << messages.at(i).GetContent().data()
                << std::endl;
        std::cout << std::endl;
    }
